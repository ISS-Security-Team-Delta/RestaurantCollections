...
describe 'Test Collaborator Handling' do
  ...
  before do
    ...
    @another_account = RestaurantCollections::Account.create(@another_account_data)
  ...
  describe 'Adding collaborators to a restaurant' do
    it 'HAPPY: should add a valid collaborator' do
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      put "api/v1/restaurants/#{@rest.id}/collaborators", req_data.to_json
  ...
  describe 'Removing collaborators from a restaurant' do
    it 'HAPPY: should remove with proper authorization' do
      @rest.add_collaborator(@another_account)
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      delete "api/v1/restaurants/#{@rest.id}/collaborators", req_data.to_json
